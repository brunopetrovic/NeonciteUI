...
for (const file of expectedRegistry) {
  if (!fs.existsSync(path.join(registryDir, file)))
    throw new Error(`[generated] missing public/r/${file}`);
}
...
for (const file of expectedPackageComponents) {
  if (!fs.existsSync(path.join(packageComponentsDir, file)))
    throw new Error(`[generated] missing packages/ui/src/components/${file}`);
}
for (const file of fs.readdirSync(packageComponentsDir)) {
  if (!/\.[cm]?[jt]sx?$/.test(file)) continue;
  if (!expectedPackageComponents.has(file))
    throw new Error(`[generated] stale packages/ui/src/components/${file}`);
}
...
if (index.items.length !== expectedCount) {
  throw new Error(
    `[generated] registry index contains ${index.items.length} items, expected ${expectedCount}`,
  );
}

console.log(
  `[generated] validation passed (${items.length} UI, ${blocks.length} blocks, ${themes.length} themes)`,
);
