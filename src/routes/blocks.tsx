  ...
  head: () => ({
    meta: [
      { title: "Blocks — Neoncite/UI" },
      {
        name: "description",
        content:
          "Installable Neoncite application and marketing sections composed from canonical primitives.",
      },
...
const blocks = [
  ...
  {
    slug: "pricing-tiers-alternative",
    label: "09 · Pricing — Alternative",
    component: PricingTiersAlternative,
  },
      ...
      <main className="flex-1 mx-auto w-full max-w-[1200px] px-4 md:px-8 py-12 md:py-16 space-y-16">
        <header>
          ...
          <h1 className="text-[36px] md:text-[56px] font-mono font-bold tracking-tighter neon-white mb-3">
            Installable composed sections
          </h1>
          <p className="text-[15px] text-muted-foreground max-w-2xl leading-relaxed">
            Every Block below is a real{" "}
            <code className="font-mono text-foreground">registry:block</code> item. Copy the install
            command, inspect its registry JSON, then replace the clearly labeled demo data with your
            product content.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {blocks.map((block) => (
              <a
                key={block.slug}
                href={`#${block.slug}`}
                className="rounded-[8px] border border-[color:var(--hairline)] px-2.5 py-1 font-mono text-[10px] text-muted-foreground hover:bg-white/5 hover:text-foreground"
              >
                {block.slug}
              </a>
            ))}
        ...
        {blocks.map((block) => {
          ...
          return (
            <BlockFrame key={block.slug} slug={block.slug} label={block.label}>
              <Preview />
            </BlockFrame>
          );
        })}
