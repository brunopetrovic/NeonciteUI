    ...
    <div className="min-h-screen flex flex-col bg-[color:var(--surface-0)]">
      ...
      <main className="flex-1 mx-auto w-full max-w-[1400px] px-4 md:px-8 py-12 md:py-16">
        <header className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-widest neon-purple mb-3">Visual</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => copy(theme.generateShareUrl(), "Share URL copied")}>
              <Link2 className="h-3.5 w-3.5" /> Share
            </Button>
            <Button variant="primary" size="sm" onClick={() => copy(theme.generateCSS(), "CSS copied")}>
        ...
        {notice && (
          <div className="fixed bottom-5 right-5 z-50 rounded-[10px] border border-[color:var(--hairline)] bg-[color:var(--surface-2)] px-4 py-2 font-mono text-[11px] text-foreground shadow-[var(--machined-shadow)]" role="status">
            {notice}
          </div>
        )}
        ...
        <section className="mb-10">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div>
              <h2 className="font-mono text-[12px] uppercase tracking-widest text-muted-foreground">Presets</h2>
              <p className="mt-1 text-xs text-muted-foreground">All official presets remain intentionally dark.</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            {THEME_PRESETS.map((preset) => (
              ...
              >
                ...
                <div className="font-mono text-[12px] font-semibold text-foreground">{preset.name}</div>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_440px]">
          <div className="space-y-6">
            <section className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] p-6">
              <h2 className="mb-5 font-mono text-[12px] uppercase tracking-widest text-foreground">Accent colors</h2>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {neonColors.map((color) => (
                        <button
                          ...
                          style={{ backgroundColor: color.value, boxShadow: field.value === color.value ? `0 0 12px ${color.value}80` : "none" }}
                        />
                      ))}
                    </div>
                  </div>
            ...
            <section className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] p-6">
              <h2 className="mb-5 font-mono text-[12px] uppercase tracking-widest text-foreground">Surface scale</h2>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                {[
                  ...
                  <label key={field.label} className="block">
                    <span className="mb-2 block font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{field.label}</span>
                    <span className="flex items-center gap-2">
                      ...
                      />
                      <code className="font-mono text-[10px] text-muted-foreground">{field.value}</code>
                    </span>
                  </label>
                ))}
              </div>
              ...
              <div className="max-w-md">
                <div className="mb-2 flex justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                  <span>Radius</span><span>{theme.radius[0]}rem</span>
                </div>
                <Slider
                  value={theme.radius}
                  onValueChange={(value) => {
            ...
            <section className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] p-6">
              <div className="mb-5 flex items-center gap-2">
                ...
                <h2 className="font-mono text-[12px] uppercase tracking-widest text-foreground">Contrast checks</h2>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {theme.contrastChecks.map((check) => {
                  const passes = check.ratio >= check.threshold;
                  return (
                    <div key={check.label} className="flex items-center justify-between gap-3 rounded-[10px] border border-[color:var(--hairline)] bg-[color:var(--recessed-bg)] px-3 py-2">
                      <span className="text-xs text-muted-foreground">{check.label}</span>
                      <span className={`font-mono text-[10px] ${passes ? "text-[color:var(--neon-green)]" : "text-[color:var(--neon-red)]"}`}>
            ...
            <section className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] p-6">
              <h2 className="mb-4 font-mono text-[12px] uppercase tracking-widest text-foreground">Saved themes</h2>
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input value={themeName} onChange={(event) => setThemeName(event.target.value)} placeholder="Theme name" />
                <Button
                  variant="outline"
                  onClick={() => {
                    try {
              ...
              {theme.savedThemes.length > 0 && (
                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  {theme.savedThemes.map((saved) => (
                    <div key={saved.name} className="flex items-center gap-2 rounded-[10px] border border-[color:var(--hairline)] px-3 py-2">
                      <button className="min-w-0 flex-1 truncate text-left font-mono text-[11px] text-foreground" onClick={() => theme.loadSavedTheme(saved.name)}>
                        {saved.name}
                      </button>
                      <button aria-label={`Delete ${saved.name}`} className="text-muted-foreground hover:text-[color:var(--neon-red)]" onClick={() => theme.deleteSavedTheme(saved.name)}>
            ...
            <div className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] overflow-hidden">
              ...
              <div className="p-5 space-y-5 bg-[color:var(--surface-0)]">
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button variant="neon" size="sm">Connect</Button>
                  <Button variant="default" size="sm">Settings</Button>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Badge variant="green">Online</Badge><Badge variant="yellow">Degraded</Badge><Badge variant="red">Down</Badge>
                </div>
                <Card>
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5"><Label htmlFor="theme-host">Hostname</Label><Input id="theme-host" placeholder="cluster-01.local" /></div>
                  <div className="flex items-center gap-5 text-[12px]"><label className="flex items-center gap-2"><Checkbox defaultChecked /> Telemetry</label><label className="flex items-center gap-2"><Switch defaultChecked /> Restart</label></div>
                </div>
              </div>
            </div>

            <div className="rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--surface-1)] p-4">
              <h2 className="mb-3 font-mono text-[11px] uppercase tracking-widest text-muted-foreground">Export</h2>
              <div className="grid gap-2">
                <Button variant="outline" size="sm" className="justify-start" onClick={() => downloadText("neoncite-theme.css", theme.generateCSS(), "text/css")}><Download className="h-3.5 w-3.5" /> CSS</Button>
                <Button variant="outline" size="sm" className="justify-start" onClick={() => downloadText("neoncite-theme.json", theme.generateJSON(), "application/json")}><Download className="h-3.5 w-3.5" /> Neoncite JSON</Button>
                <Button variant="outline" size="sm" className="justify-start" onClick={() => downloadText("neoncite-theme.dtcg.json", theme.generateDTCG(), "application/json")}><Download className="h-3.5 w-3.5" /> DTCG JSON</Button>
              </div>
            </div>

            <pre className="max-h-[300px] overflow-auto rounded-[16px] border border-[color:var(--hairline)] bg-[color:var(--recessed-bg)] p-4">
              <code className="whitespace-pre font-mono text-[10px] leading-relaxed text-foreground">{theme.generateCSS()}</code>
            </pre>
          </aside>
        </div>
      </main>
