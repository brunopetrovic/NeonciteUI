    ...
    <section id={slug} className="scroll-mt-24">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          type="button"
          onClick={copy}
          className="inline-flex h-7 items-center gap-1.5 rounded-[8px] border border-[color:var(--hairline)] px-2.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground hover:bg-white/5 hover:text-foreground"
        >
          {copied ? (
            <Check className="h-3 w-3 text-[color:var(--neon-green)]" />
          ) : (
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          aria-label={`Open ${slug} registry JSON`}
          className="inline-flex h-7 w-7 items-center justify-center rounded-[8px] border border-[color:var(--hairline)] text-muted-foreground hover:bg-white/5 hover:text-foreground"
        >
